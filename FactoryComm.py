#python3.11  C:\Python311\Lib\site-packages\PyInstaller FactoryComm.py --onefile
...
from FH_Omron_Vision import *
from PyHistory import *
...
LEVEL_TRACKING = "TRACKING"
LEVEL_WARNING = "WARNING"
LEVEL_NORMAL = "NORMAL"
LEVEL_ERROR = "ERROR"

HistorySystem = HistoryLog("FactoryComm_System_Log")
HistorySystem.insert(TOPIC_SYSTEM,LEVEL_NORMAL,"APPLICATION STARTUP")
        ...
        try:
            for self.transaction in event["command"]:
                try:
                    ...
                    event["ready"] = False
                    HistoryService = HistoryLog("Service-Fins-Log-")
                    HistoryService.insert(TOPIC_SERVICE, LEVEL_ERROR,"[" + str(traceback.extract_stack()[-1][1]) + "][" + str(e) + "]")
        except Exception as e :
            ...
            try:
                for self.transaction in event["command"]:
                    try:
                        ...
                        self.transaction["except"] = str(e)
                        event["ready"] = False
                        HistoryService = HistoryLog("Service-Ping-Log-")
                        HistoryService.insert(TOPIC_SERVICE, LEVEL_ERROR,"[" + str(traceback.extract_stack()[-1][1]) + "][" + str(e) + "]")
                ...
                while True:
                    for target in ip_group:
                        try:
                            ...
                            HistoryService = HistoryLog("Service-Ping-Log-")
                            HistoryService.insert(TOPIC_SERVICE,LEVEL_ERROR,"["+str(traceback.extract_stack()[-1][1])+"]["+str(ex)+"]")
                    time.sleep(1)
...
class mssql:
    def __init__(self, event):
        try:
            ...
            event['ready'] = False
            event['except'] = str(ex)
            HistoryService = HistoryLog("Service-Mssql-Log-")
            HistoryService.insert(TOPIC_SERVICE, LEVEL_ERROR,"[" + str(traceback.extract_stack()[-1][1]) + "][" + str(ex) + "]")
...
class writefile:
    def __init__(self, event):
        ...
        try:
            ...
            event["ready"] = False
            event["except"] = str(ex)
            HistoryService = HistoryLog("Service-WriteFile-Log-")
            HistoryService.insert(TOPIC_SERVICE, LEVEL_ERROR,"[" + str(traceback.extract_stack()[-1][1]) + "][" + str(ex) + "]")
    # Package Example
    # {
    #    "service": "writefile",
...
class readfile:
    def __init__(self, event):
        try:
            ...
            event["ready"] = False
            event["except"] = str(ex)
            HistoryService = HistoryLog("Service-ReadFile-Log-")
            HistoryService.insert(TOPIC_SERVICE, LEVEL_ERROR,"[" + str(traceback.extract_stack()[-1][1]) + "][" + str(ex) + "]")
    ...
    def __init__(self, event):
        ...
        for self.transaction in event["command"]:
            try:
                ...
                self.transaction["except"] = str(e)
                event["ready"] = False
                HistoryService = HistoryLog("Service-ModbusTCP-Log-")
                HistoryService.insert(TOPIC_SERVICE, LEVEL_ERROR,"[" + str(traceback.extract_stack()[-1][1]) + "][" + str(e) + "]")
...
class dir:
    ...
    def __init__(self, event):
        try:
            ...
            event["except"] = str(e)
            event["ready"] = False
            HistoryService = HistoryLog("Service-Dir-Log-")
            HistoryService.insert(TOPIC_SERVICE, LEVEL_ERROR,"[" + str(traceback.extract_stack()[-1][1]) + "][" + str(e) + "]")
...
class pf6000:
    ...
    def decode_open_protocol(self,source):
        ...
        try:
            ...
            HistoryService = HistoryLog("Service-PF6000-Log-")
            HistoryService.insert(TOPIC_SERVICE, LEVEL_ERROR,"[" + str(traceback.extract_stack()[-1][1]) + "][" + str(error) + "]")
            return False, None
...
class stream:
    ...
    def service(self,event):
        ...
        try:
            ...
            event["exception"] = str(e)
            event["ready"] = False
            HistoryService = HistoryLog("Service-Stream-Log-")
            HistoryService.insert(TOPIC_SERVICE, LEVEL_ERROR,"[" + str(traceback.extract_stack()[-1][1]) + "][" + str(e) + "]")
...
def server():
    ...
    HistorySystem.insert(TOPIC_ADMIN,LEVEL_NORMAL,"INIT SERVICE " + str(hostaddr) + ":" + str(port))

    print("**************************************************************************************************")
    print("Software : FactoryComm")
    print("Server : " + str(hostname) + "\r")
    print("Service Path : " + str(hostaddr) + ":" + str(port) + "\r")
    print("API Verison : 2.7" + "\r")
    print("**************************************************************************************************")
    s = socket.socket()
    ...
    HistorySystem.insert(TOPIC_SYSTEM,LEVEL_NORMAL,"API STARTED")
    while True:
        # package test mssql
        # {
        #    "client": {"addr": "", "port": ""},
        ...
        if not recive:
            ...
            try:
                ...
                #print("client : " + str(client[0]) + ":" + str(client[1]) + " Time : " + datetime.datetime.now().strftime("%d/%m/%Y, %H:%M:%S"))
                #print("method : " + method + " , url : " + url )
                #print("Read content length : " + str(len(data)) + " Bytes")
                uid = json.loads(data)
                uid['tstart'] = datetime.datetime.now().strftime("%d/%m/%Y, %H:%M:%S")
                uid['client']['addr'] = client[0]
                uid['client']['port'] = client[1]
                eventcount=0

                HistorySystem.insert(TOPIC_SYSTEM,LEVEL_TRACKING,"REQUEST " + str(client[0]) + ":" + str(client[1]) + " SIZE " + str(len(data)) +" Bytes")
                ...
                for event in uid['events']:
                    if event['service'] == "mssql":
                        ...
                        #            }
                        drive.service(event)
                    elif event['service'] == "omronFH_Vision":

                            FH_Camera_Index = 0
                            FH_Camera_Check = False

                            if len(omronFH_Group) == 0 :
                                omronFH_Group.append( FH_Omron_Vision(event['connection']['source']))

                            for FH_Camera in omronFH_Group :
                                    if event['connection']['source'] == FH_Camera.ip :
                                         FH_Camera_Check= True
                                         break
                                    else:
                                         FH_Camera_Index=FH_Camera_Index+1

                            if FH_Camera_Check == True :
                                  omronFH_Group[FH_Camera_Index].req(event)
                ...
                uid['tend'] = datetime.datetime.now().strftime("%d/%m/%Y, %H:%M:%S")
                SendbackToClientAndCloseConnection(res, uid)
                HistorySystem.insert(TOPIC_SYSTEM,LEVEL_TRACKING,"REPLY " + str(client[0]) + ":" + str(client[1]) + " SIZE " + str(len(json.dumps(uid))) +" Bytes")
            except Exception as ex:
                HistorySystem.insert(TOPIC_SYSTEM,LEVEL_ERROR,"["+str(traceback.extract_stack()[-1][1])+"]["+str(ex)+"]")
...
HistorySystem.insert(TOPIC_SYSTEM, LEVEL_NORMAL, "ENABLE SERVICE BACKUP PROCESS")
while (True):
    if (PID_server.is_alive() == False):
            PID_server = Thread(target=server)
            PID_server.run()
            HistorySystem.insert(TOPIC_SYSTEM, LEVEL_WARNING,"RESTORE SERVICE SERVER")

    if (PID_ping.is_alive() == False):
            PID_ping = Thread(target=server_ping)
            PID_ping.run()
            HistorySystem.insert(TOPIC_SYSTEM, LEVEL_WARNING,"RESTORE SERVICE PING")

    #Check Camera Process is Down New Start Thread
    if len(omronFH_Group) > 0 :
            for camera in range(len(omronFH_Group)-1):
                    omronFH_Group[camera].Protect_ProcessDown()
